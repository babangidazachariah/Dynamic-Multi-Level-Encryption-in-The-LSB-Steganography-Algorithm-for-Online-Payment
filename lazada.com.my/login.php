...
SESSION_START();
	...
	if(isset($_POST['btnSubmit'])){
		...
		if(empty($errorMessage )){
			...
			$query = "SELECT registeredBank, registeredAccountNumber FROM tblRegisteredCustomers WHERE regiteredEmail 
				='". $email."' AND registeredPassword ='".$password."'";
			$mysqli = connection();
			$result = $mysqli->query($query);
			
			if($result->num_rows > 0){
				while($row = $result->fetch_assoc()){
					$_SESSION['bank'] = $row['registeredBank'];
					$_SESSION['accountNumber'] = $row['registeredAccountNumber'];
				}
...
<html>
<head>
<title>LAZADA - Costomer Login</title>
<style type="text/css">
    ...
    <div id="content">
		<form method="POST" action="login.php">
			...
			<div id="formControl">
				...
				<input type ="submit" name ="btnSubmit" id="btnSubmit" value="Login" class="btnSubmit"  />
			</div>
		</form>
    </div>
</div>
